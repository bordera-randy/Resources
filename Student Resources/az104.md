...
oCan provide Bash or PowerShell
...
oBilling & cost managements


.On Portal

oYou can search for Tags and see filtered lists.
...
oIf spending limit is exceeded:
...
o E.g. Microsoft.Compute that handles VMs, Microsoft.Network, Microsoft.Sql, Microsoft.Storage 


. Some are not registered automatically 

o E.g. Microsoft.PolicyInsights, Microsoft.AzureActiveDirectory, Microsoft.AzureStack, Microsoft.Botservice 
...
o By default, for a new subscription, the Account Administrator is also the Service Administrator. 

o The Service Administrator has the equivalent access of a user who is assigned the Owner role at the subscription scope. 
...
o The Co-Administrator has the equivalent access of a user who is assigned the Owner role at the subscription scope. 
...
o Microsoft Graph: API product trying to creating single way of interacting with all Microsoft APIs. 
...
o You can create custom that are application specific and are outside of the direct administration of Azure AD 


. Roles are assigned at tenant level. 


o If you need separation of roles, you can create a new tenant and assign roles and permissions on that account. 
...
o . If you're global admin in one directory doesn't mean you have any access in other directory. 
...
o User risk policy: E.g. if user risk event is high, allow access but require password change 
...
o Owners manage groups instead of administrator that manage the group for the owners. 
...
o Allows you to customize the pages with e.g. banner, sign-in page text, user name hint 
...
o If they're already joined to local AD, they're also registered in Azure AD automatically. 
...
o A common approach to building applications, and full flexibility to deploy in the cloud or on-premises 


. Integrated management and security 

o Built-in management and security solutions across full operational lifecycle from cloud to on-premises 


. Common Identity 

o Enable end-user productivity with single sign on to cloud and on premises applications while protecting corporate data 
...
o Does your company have more than one user with elevated privilege to manage your identity system? 
...
o Do you have an account with the correct permissions for all forests you want to synchronize with? 
...
o Does your company have any application that needs custom control access to some resources? 
...
o Useful when e.g. some users passwords cannot be written back to the cloud but another department can do it. 

o . Azure AD Connect sync servers must be configured for mutually exclusive filtering. 

o . Users in one Azure AD will only be able to see users from their own Azure AD instance. 
...
o User is authenticated using pass through authentication agent that connects with on-premises AD 
...
o Requires Azure service endpoints to be added to the client browser's Intranet zone. 
...
o . Publishing requires Microsoft.Authorization/policyassigments/write permission. 
...
o E.g.: Require SQL Server 12.0, Allowed Storage Account SKU, Allowed Resource Types, Allowed Locations, Allowed Virtual Machine SKUs, Apply tag and its default value, Enforce tag and its value, Not allowed resource types 
...
o .. You can also see daily usage data in Portal: Azure Account Center . Billing history . Current period . Download usage 
...
o Helps to design, plan, deliver, operate, and control information technology (IT) services 
...
o Based on each query that run on regular intervals, results are evaluated to trigger an alert. 
...
o Signifies an agreed-upon description of product attributes, per unit time, which serves as a basis for defining change. 
...
o E.g. configure a policy to move a blob directly to the archive storage tier X days after it's uploaded 
...
o Access Policy . Add Policy . Time-based retention (set retention period) / Legal hold (attach tags) . Lock policy 
...
o CDN can authenticate to Blob Storage with SAS tokens to be able to read private data. 
...
o Geo-spatial search if index data has latitude and longitude, user can get related data based on proximity 
...
o You can contact Azure support and request higher limits (as of now 5 PB). Same for ingress/egress limits to. 
...
o . You don't get to choose second region, they're paired regions decided by Microsoft. 
...
o Manage SAS, CORS, access levels, meta data, files in File Share, stored procedures in Cosmos DB 
...
o Signature is signed with account key and ensures none of the parameters are tempered. 
...
o SMB 2.1 does not have encryption so it's only allowed between different Azure regions. 
...
o When uploading encrypted VM, you can upload encryption keys to Azure Key Vault first. 
...
o Encryption used if outside the Azure region or if required as part of the storage account configuration 

o SMB access utilizes the storage account name (as user name) and access key (as password). 
...
o .. Content should be restored to an IaaS VM file server to enable ACL enforcement. 
...
o In Sync Services . Sync Groups . Group . See health status and action recommendations for problems for cloud and server endpoints 
...
o Vault . Backup Alerts . Configure notifications . Enable e-mail notifications . Choose severities (critical, warning, information) . Select notification (per alert or hourly digest) 
...
o You can save reports in you can archive reports in storage accounts, stream to event hubs, send to Log Analytics 

o After you configure a storage account for reports by using a Recovery Services vault, you can connect Azure Backup from Power BI and get a dashboard. 
...
o Each and every recovery point it has information for what it needs to go back to recovery point 
...
o Protected instance is an application server/workload or computer that's been configured to back up to Microsoft Azure 
...
o Workloads, VMWare and Hyper-V VMs, hosts, files, application workloads and barebone backups 
...
o E.g.   virtual network for the network interface card   all other network interface cards for the virtual network   VPN gateways 
...
General purpose 
 ...
 Balanced CPU-to-memory ratio. Ideal for testing and development, small to medium databases, and low medium traffic web servers. 
...
GPU 
 NV, NC, NCv2, NCv3, ND 
 Specialized virtual machines targeted for heavy graphic rendering and viedo editing, as well as model 
...
training and inferencing (ND) with deep learning. Available with single or multiple GPUs 
...
o Standard SSD available for managed disks (dev/test/entry elvel production applications) 
...
o Azure manages the disks, you don't have to worry about storage account-level IOPS restrictions. 
...
o . You can resize only when they're unattached or owner VM is stopped & deallocated 
...
o VM -> Overview -> Configure DNS Name then you can have like somename.eastus2.cloudapp.azure.com 
...
o Enable VM backup to protect your data from corruption and accidental deletion (medium impact) 
...
o   Azure portal   Azure Cloud Shell   Azure PowerShell   Azure CLI   Azure SDKs   ARM templates 
...
o Useful if you want to customize the VM, add configuration settings that weren't there at the time of backup, or add settings that must be configured using the template or PowerShell. 
...
o . Not supported for unmanaged disks, generalized VMs, or for VMs created using custom images. 
...
o Select target resources (e.g. VNEt, availability set, RG) to new resources or existing 
...
o To move a peered virtual network, you must first disable the virtual network peering 

o . You can't move a virtual network to a different subscription if the virtual network contains a subnet with resource navigation links 
...
o Uses CIDR block of private RFC 1918 addresses that are not public/internet routable themselves 
...
o E.g. all of your web front-ends will have similar access requirements, then you can bind NSGs on subnet level. 
...
o Subnet: Default gives you one subnet, for more you can use ARM template or PowerShell/CLI 
...
o E.g. Internet, VirtualNetwork, AzureLoadBalancer, AzureTrafficManager, Storage, SQL, AzureCosmosDB, AzureKeyVault. 
...
o E.g. if you have network appliance and you want to give it ability to forwar traffic that's not destined for itself 
...
o You can use user defined routes for all outbound traffic to go back through VPN gateway to on-premises. 


. . Peerings are not transitive 


o If you peer spoke1 <=> spoke2 and spoke2 <=> spoke3 then spoke1 cannot communicate with spoke3 automatically. 
...
o Network Watcher and Network Permormance Monitor make troubleshooting much easier 
...
o Both gateways gives access to front-end tier and a jumpbox in a management subnet 
...
o E.g. configure DNS forwarding between one DNS server in one VNet to another DNS server in another VNet 
...
o Set-up DNS name for peered Virtual Appliance and a VNet in a Hub & Spoke topology 
